...
var Draggable = Class.create({
  ...
  finishDrag: function(event, success) {
    ...
    if(this.options.zindex)
      /* patched by blair for treecms { */
      this.element.style.zIndex = null;//this.originalZ;
      /* } */
...
var Sortable = {
  ...
  markEmptyPlace: function(element) {
    if(!Sortable._emptyPlaceMarker) {
      ...
      /* patched by Blair { */
      if (Sortable.options(element).dropZoneClass != null)
        Element.addClassName(Sortable._emptyPlaceMarker, Sortable.options(element).dropZoneClass);
      else
    ...
    /* patched by Blair - gives a slightly wider or taller drop zone { */
    var horizontalPadding = (Sortable.options(element).overlap == 'vertical') ? ((Element.getDimensions(element).width)*.01) : 0;
    var verticalPadding = (Sortable.options(element).overlap == 'horizontal') ? ((Element.getDimensions(element).height)*.01) : 0;
    /* } */
    
    Sortable._emptyPlaceMarker.style.left = (pos[0]) - (horizontalPadding/2) + 'px';
    Sortable._emptyPlaceMarker.style.top = (pos[1]) - 1 - (verticalPadding/2) + 'px';
	if(md != null) 
	    Sortable._emptyPlaceMarker.style.margin = md;

    var d = {};
    
    d.width = (Element.getDimensions(element).width) + horizontalPadding + 'px';
    d.height = (Element.getDimensions(element).height) + verticalPadding + 'px';
