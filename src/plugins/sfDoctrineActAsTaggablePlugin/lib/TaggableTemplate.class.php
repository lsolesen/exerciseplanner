        ...
        if (!isset($option) || !$option->hasNamespace('saved_tags'))
        {
            ...
            else
            {
                $saved_tags = Doctrine_Query::create()
                                            ...
                                            ->where('t.lang = ? AND tg.taggable_id = ? AND tg.taggable_model = ?')
                                            ->execute(array(sfContext::getInstance()->getUser()->getCulture(),$this->getInvoker()->id, get_class($this->getInvoker())),
